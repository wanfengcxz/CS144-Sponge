...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    _len += _output.bytes_read() - _first_unread;   // 读出去了多长的字节 需要在装配缓冲区同样扩展出这段长度
    ...
    while(data_start < data_len && offset < _len) {
        size_t tmp = (_start + offset) % _capacity;                                     // _unassembled是一个循环队列，需要取模来得到下标
        _unassembled[tmp] = data[data_start];
        if (_flag[tmp] != 1) ++_unassembled_len;                                        // 如果当前位置不是覆盖，则未重组元素个数+1
        _flag[tmp] = 1;                                                                 // 标记当前下标已经放有字符
        ++data_start, ++offset;
    }
