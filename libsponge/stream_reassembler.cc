...
StreamReassembler::StreamReassembler(const size_t capacity)
    : StreamReassembler(capacity, std::vector<char>(capacity), std::vector<char>(capacity)) {}
...
void StreamReassembler::push(size_t idx, char ch) {
    _unassembled[idx] = ch;
    if (_flag[idx] != 1)
        ++_unassembled_len;  // 如果当前位置未有字符填充，则未重组元素个数+1
...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    ...
    if (_eof)
        data_len =
            std::min(data_len, _end_unassembled - index - 1);  // 如果EOF已经指定过，那么需要对data进行截断，防止越过EOF
    size_t data_start = std::max(_first_unassembled, index) -
                        index;  // 当前index可能在_first_unassembled之前，则需要把前面的已经重组好的部分截断

    size_t offset = index > _first_unassembled ? index - _first_unassembled
                                               : 0;  // offset是往未重组缓冲区_unassembled写入时，下标的偏移
    ...
    if (eof) {
        _eof = true;
        _end_unassembled = std::max(
            index + data_len, _first_unassembled);  // TODO：如果index+data_len在_first_unassembled之前到底怎么处理？
    }
    ...
    if (_eof && _first_unassembled >= _end_unassembled)
        _output.end_input();  // 此次写入的连续字符串刚好包含了EOF位置，则表示数据接收完毕。
}
